...
businessRoutes.route('/add').post((req, res) => {
    ...
    if(req.body.toString() != '') {
        business.save().then(business => {
            res.status(200).json({ 'business': 'business is added successfully', 'data': req.body });
        }).catch(err => {
                res.status(400).send('Unable to add  business to database');
        });
    } else {
        res.status(500).send('request body kosong');
    }
});
...
businessRoutes.route('/test').get((req,res) => {
    Business.find(function (err, business) {
        if (business) {
            res.json(business.toString());
        } else {
            console.log(err);
        }
    });
